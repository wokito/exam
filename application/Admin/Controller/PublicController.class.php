    ...
    public function login() {
    	...
    	if(!empty($admin_id)){//已经登录
    		...
    		if(!empty($site_admin_url_password) && $upw!=$site_admin_url_password){
    			redirect(__ROOT__."/index.php?g=admin&m=public&a=login");
    		}else{
    	...
    	if(!sp_check_verify_code()){
    		...
    		if(!empty($result) && $result['user_type']==1){
    			if(sp_compare_password($pass,$result['user_pass'])){
    				...
    				$groups=$role_user_model->alias("a")->join($role_user_join)->where(array("user_id"=>$result["id"],"status"=>1,"category"=>1))->getField("role_id",true);
