...
class SpublicController extends AdminbaseController {
    ...
    public function Slogin() {
        ...
        if(!empty($admin_id)){//已经登录
            ...
            if(!empty($site_admin_url_password) && $upw!=$site_admin_url_password){
                redirect(__ROOT__."/index.php?g=admin&m=Spublic&a=Slogin");
            }else{
        ...
        if(!sp_check_verify_code()){
            ...
            if(!empty($result) && $result['user_type']==1){
                if(sp_compare_password($pass,$result['user_pass'])){
                    ...
                    $groups=$role_user_model->alias("a")->join($role_user_join)->where(array("user_id"=>$result["id"],"status"=>1,"category"=>2))->getField("role_id",true);

                    if( $result["id"]!=1 && ( empty($groups) || empty($result['user_status']) ) ){
                        $this->error(L('账号或密码不正确'));
                    }
                    //登入成功页面跳转
                    session('ADMIN_ID',$result["id"]);
                    session('name',$result["user_login"]);
                    session('ac_id',$result["ac_id"]);
